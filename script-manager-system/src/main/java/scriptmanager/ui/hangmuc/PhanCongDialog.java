...
public class PhanCongDialog extends JDialog {
    ...
    private void addThietBi() {
        ...
        if (sl <= 0) {
            JOptionPane.showMessageDialog(PhanCongDialog.this, "Số lượng phải > 0", "Lỗi", JOptionPane.ERROR_MESSAGE);
            return;
        }
        
        if (sl > item.tb.getSoLuong()) {
            JOptionPane.showMessageDialog(PhanCongDialog.this, "Vượt quá tồn kho (Còn: " + item.tb.getSoLuong() + ")", "Lỗi", JOptionPane.ERROR_MESSAGE);
            return;
        }
        ...
        new SwingWorker<Void, Void>() {
            @Override protected Void doInBackground() throws Exception {
                ...
                pc.setHangMuc(hm);
                pc.setThietBi(item.tb);
                pc.setSoLuongSuDung(sl);
    ...
    private void addDaoCu() {
        ...
        if (sl > item.dc.getSoLuong()) {
            JOptionPane.showMessageDialog(PhanCongDialog.this, "Vượt quá tồn kho (Còn: " + item.dc.getSoLuong() + ")", "Lỗi", JOptionPane.ERROR_MESSAGE);
            return;
        }
